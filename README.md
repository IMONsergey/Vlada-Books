...
- 177 реальных обложек сопоставлены с каталогом и лениво кэшируются в R2;
...
Индекс найденных обложек хранится в `app/data/cover-map.json`. `app/data/cover-review.json` содержит низкоуверенные и ненайденные позиции: сейчас 175 совпадений имеют высокую или среднюю уверенность, 2 требуют проверки, для 58 позиций открытая обложка не найдена. Приложение не подставляет случайные изображения вместо них.
